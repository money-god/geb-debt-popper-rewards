...
import "../MultiDebtPopperRewards.sol";

abstract contract Hevm {
    function warp(uint256) virtual public;
}
contract AccountingEngine {
    mapping (uint256 => address) pop;

    function popDebt(uint256 timestamp) external {
        pop[timestamp] = msg.sender;
    }
    function debtPoppers(bytes32, uint256 timestamp) public view returns (address) {
        return pop[timestamp];
    }
}
contract Attacker {
    function doGetRewardForPop(address popperRewards, uint256 slotTimestamp, address feeReceiver) public {
        MultiDebtPopperRewards(popperRewards).getRewardForPop(slotTimestamp, feeReceiver);
    }
}

contract MultiDebtPopperRewardsTest is DSTest {
    Hevm hevm;

    Attacker attacker;

    MultiDebtPopperRewards popperRewards;
    ...
    function setUp() public {
        ...
        popperRewards    = new MultiDebtPopperRewards(
            coinName,
        ...
        treasury.setTotalAllowance(coinName, address(popperRewards), uint(-1));
        treasury.setPerBlockAllowance(coinName, address(popperRewards), fixedReward * 10E27);
    }
    ...
    function test_setup() public {
        ...
        assertEq(popperRewards.coinName(), coinName);
    ...
    function testFail_get_reward_when_total_allowance_zero() public {
        treasury.setTotalAllowance(coinName, address(popperRewards), 0);
        hevm.warp(now + 2 weeks + 1);
        uint slotTime = now;
        accountingEngine.popDebt(slotTime);
        hevm.warp(now + 1);
    ...
    function testFail_get_reward_per_block_allowance_zero() public {
        treasury.setPerBlockAllowance(coinName, address(popperRewards), 0);
        hevm.warp(now + 2 weeks + 1);
        uint slotTime = now;
        accountingEngine.popDebt(slotTime);
        hevm.warp(now + 1);
